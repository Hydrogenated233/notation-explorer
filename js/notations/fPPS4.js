;
/**
 * Parented Predecessor Sequence 4 fourth (fPPS4)
 *
 * 在 tPPS4 基础上修改：强展开时，末项第一次复制为坏根列标（即原末项的值x），
 * 后面同tPPS4（加阶差）。
 *
 ...
 *
 * 其他项展开（强展开时）：由末项复制出的项（位置 y+L, y+2L, ...）中，
 * 第一次复制（位置 y+L）的值 = x（坏根列标/原末项值），
 * 后续复制（位置 y+2L, y+3L, ...）同tPPS4：值 = x + m*L（m从1开始）。
 *
 * 基本列[n]：展开到第 y + n*L 项
 *
 * 例子:
...
(function () {
  ...
  function expand(seq, FSterm) {
    ...
    if (isStrong) {
      // fPPS4 强展开：
      // 末项（位置 y）的值 = v（坏根列标指向的列标）
      // 末项第一次复制（位置 y+L）= x（原末项值）
      // 后续末项副本（位置 y+2L, y+3L, ...）= x + m*L
      ...
      for (var pos = y + 1; pos <= totalLen; pos++) {
        ...
        if (isCopyOfLast) {
          var m = (pos - y) / L; // m=1,2,3,...
          // m=1（第一次复制）: 值 = x
          // m>=2（后续复制）: 值 = x + (m-1)*L
          result.push((m === 1) ? x : x + (m - 1) * L);
        } else {
          // 普通PPS4规则
          var srcPos = pos - L;
  ...
  register.push({
    ...
    fromDisplay: function (str) {
      ...
      if (result.find(Number.isNaN) !== undefined) throw new Error("Illegal fPPS4 sequence");
      return result;
    },
    able: function (seq) {
